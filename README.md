...
Этот мини-проект позволяет автоматически загружать файлы, отправленные Telegram-боту, на ваше хранилище Yandex.Disk, очищать корзину, и искать файлы по запросу пользователя. Файлы будут загружаться в автоматически создаваемые папки по типу 'August_2024'. В созданной папке по времени будет подсортировка по типу загружемого файла (Фото, Видео, Аудио, и прочие файлы). Функционал бота будет постепенно обновляться и расширяться.
...
2. Создайте по инструкции нового бота и получите API-токен
...
Если вы пользователь Windows, то можете легко установить Python через [официальный сайт](https://www.python.org/downloads/).
...
1. Установите необходимые пакеты:
    ```bash
    sudo apt-get install git python3.x-venv # Где x - последняя доступная версия Python в вашей системе, например python3.10 и выше.
    ```
...
Теперь установите необходимые зависимости из файла requirements.txt, находясь в виртуальном окружении:
```bash
...
```
4. Скопируйте полученный `chat_id` и добавьте его в `.env` файл в переменную CHAT_ID.

### Шаг 9: Запуск бота
Теперь почти всё готово, осталось лишь запустить бота:
```bash
